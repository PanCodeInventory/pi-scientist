...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			const discovery = discoverScientists();
			const effectiveCwd = params.cwd ? path.resolve(ctx.cwd, params.cwd) : ctx.cwd;
			const task = `Read the plan file at \`${params.planFile}\`. Effective analysis parent directory: ${effectiveCwd}. Identify the latest step that the worker executed (it will still be marked \`- [ ]\` because the worker is forbidden from modifying the plan). Review its outputs for code correctness, statistical validity, figure quality (MUST first read skills/visualization/shared/figure-standards.md, then run its 6 BLOCKER checks — any failure is NEEDS FIX), data provenance, and plan compliance. If PASS: mark the checkbox \`- [x]\` in the Todolist and optionally add review notes. If NEEDS FIX: add concrete fix steps to the Todolist and Task Details sections. Update the plan file accordingly.`;
