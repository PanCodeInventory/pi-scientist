...
def main():
    ...
    # ──────────────────────────────────────────────────
    # Data Storage Standard: Save raw counts FIRST (before any transformation)
    # MUST use .copy() — without it, layers["counts"] is a reference, not a copy
    # ──────────────────────────────────────────────────
    adata.layers["counts"] = adata.X.copy()
    print(f"Saved raw counts to adata.layers['counts'] (shape: {adata.layers['counts'].shape})")
    ...
    # ──────────────────────────────────────────────────
    # Data Storage Standard: Save log-normalized snapshot before HVG subset
    # Note: This script only does QC + filtering, normalization is done later.
    # If normalizing here, do: normalize → log1p → adata.raw = adata
    # ──────────────────────────────────────────────────
    # Uncomment below if this script also performs normalization:
    ...
    if adata.raw is not None:
        ...
        print("adata.raw: not set (set after normalize + log1p in downstream pipeline)")
